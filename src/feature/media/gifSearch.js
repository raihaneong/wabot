...
const { logDate } = require("../../utils/log");
...
async function gifSearch(chat, msg, query) {
  ...
  try {
    ...
    const response = await axios.get(
      `https://tenor.com/search/${encodeURIComponent(query)}`,
      {
        headers: {
          "User-Agent":
    ...
    const regex =
      /https:\/\/media\.tenor\.com\/[a-zA-Z0-9_-]+\/[a-zA-Z0-9_-]+\.(mp4|gif)/g;
